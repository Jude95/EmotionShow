...
/**
 * https://github.com/whyalwaysmea/DoubleScrollView
 *
 * xml布局中写明子View的id为header、footer
 */
public class GoodsDetailLayout extends LinearLayout {
    ...
    public GoodsDetailLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy > mTouchSlop && screen == SCREEN_FOOTER) {// pull down
                    result = (pullListener != null && pullListener.footerHeadReached(ev));
                } else if (dy < -mTouchSlop && screen == SCREEN_HEADER) { // pull up
                    result = (pullListener != null && pullListener.headerFootReached(ev));
                }
                break;
